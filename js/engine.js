...
var Engine = (function(global) {
    ...
    function mainSelector() {
      renderSelection();
      if (selectPhase) {
        win.requestAnimationFrame(mainSelector);
      } else {
        init();
      }
    }
    ...
    function renderSelectionPlayers() {
      ...
      for (row = 0; row < 1; row++) {
          for (col = 0; col < numCols; col++) {
            ctx.drawImage(Resources.get(playerImages[col]), col * 101, row * 83);
          }
      }
    }
    ...
    Resources.onReady(mainSelector);  //changed from 'init' to 'MainSelector' and here we go!
    ...
    global.playerImages = playerImages;
    global.Game = {
      init: init,
